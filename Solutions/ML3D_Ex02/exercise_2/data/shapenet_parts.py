...
class ShapeNetParts(torch.utils.data.Dataset):
    ...
    def get_point_cloud_with_labels(shapenet_id):
        ...
        points_path = ShapeNetParts.dataset_path / category_id / 'points' / (shape_id + ".pts")    
        point_label_path = ShapeNetParts.dataset_path / category_id / 'points_label' / (shape_id + ".seg")

        points = np.loadtxt(points_path) # N x 3
        point_labels = np.loadtxt(point_label_path) # N x 1    

        # subsample to 1024 points and labels
        
        # if less than 1024
        if points.shape[0] < ShapeNetParts.num_points:
        ...
        # part id to overall id
        for i in range(point_labels.shape[0]):
            point_labels[i] = ShapeNetParts.part_id_to_overall_id[category_id + "_" + str(int(point_labels[i]))]

        return points.T, point_labels.astype(np.int64)
