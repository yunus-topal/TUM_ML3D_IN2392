        ...
        for i, batch in enumerate(trainloader):
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    batch_loss = torch.zeros([1], dtype=batch_val['voxel'].dtype, requires_grad=True).to(device)
                    for output_idx in range(prediction.shape[1]):
                        batch_loss = batch_loss + loss_criterion( prediction[:, output_idx, :], batch_val['label'])  # TODO: Loss due to prediction[:, output_idx, :] (output_idx=0 for global prediction, 1-8 local)
                    ...
                    loss_total_val += batch_loss.item()
