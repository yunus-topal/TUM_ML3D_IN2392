...
def train(model, train_dataloader, val_dataloader, device, config):
    ...
    loss_criterion_smooth = torch.nn.SmoothL1Loss(reduction='mean')
    ...
    for epoch in range(config['max_epochs']):
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            batch = ShapeNet.move_batch_to_device(batch, device)
            ...
            out = model(batch['input_sdf'])
            out[batch['input_sdf'][:, 1] == 1] = 0 
            loss1 = loss_criterion_smooth(out, target)
            loss2 = loss_criterion(out, target)
            loss = loss1 + loss2
            loss.backward()
            optimizer.step()
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in val_dataloader:
                    ...
                    loss1 = loss_criterion_smooth(reconstruction, target)
                    loss2 = loss_criterion(reconstruction, target)
                    loss = loss1 + loss2
                    loss_val += loss.item()
