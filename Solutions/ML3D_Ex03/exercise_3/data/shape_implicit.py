...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        pos_tensor = remove_nans(torch.from_numpy(npz["pos"])) # shape = 32 x 32 x 32
        neg_tensor = remove_nans(torch.from_numpy(npz["neg"])) # shape = 32 x 32 x 32

        pos_perm = torch.randperm(pos_tensor.shape[0])[:self.num_sample_points // 2]
        neg_perm = torch.randperm(neg_tensor.shape[0])[:self.num_sample_points // 2]

        pos_tensor = pos_tensor[pos_perm]

        neg_tensor = neg_tensor[neg_perm]
        
        # concat pos and neg tensors
